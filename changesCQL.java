...
public class CqlDelimLoad {
	...
	private String pcapFileEndStr = "ready.csv";
	...
	private boolean parseArgs(Properties args) throws IOException, FileNotFoundException {
		String tkey;
		if (args.isEmpty()) {
	...
	public boolean run(Properties args)
		...
		if (onefile) {
			...
			System.out.println("watching the directory for the changes:");
			...
			while (true) {
				...
				for (WatchEvent<?> event : key.pollEvents()) {
					...
					System.out.println("pathname is:"+kind);
					if (kind == ENTRY_CREATE) {
						if (isPcapfileReady(pathName.toFile().toString())) {
							File tFile = new File(String.valueOf(filename + "//" + pathName.toFile()));
							Callable<Long> worker = new CqlDelimLoadTask(cqlSchema, delimiter,
									nullString,
									dateFormatString,
									boolStyle, locale,
		...
		/*	while (!fileList.isEmpty()) {
				...
				System.out.println("tfile is:"+tFile);
				Callable<Long> worker = new CqlDelimLoadTask(cqlSchema, delimiter,
						nullString,
						dateFormatString,
						boolStyle, locale,
	...
	boolean isPcapfileReady(String fileName)
	{
		/*if(fileName.length() > pcapFileEndStr.length())
		{
			String endStr = fileName.substring(fileName.length()- pcapFileEndStr.length(), fileName.length());
			int x = endStr.compareTo(pcapFileEndStr);
			if(x==1)
				return true;
		}*/
		return fileName.endsWith(".ready.csv");
	}
